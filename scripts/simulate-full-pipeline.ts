/**
 * scripts/simulate-full-pipeline.ts
 ...
 *
 * Usage:
 *   npx tsx scripts/simulate-full-pipeline.ts
 */
...
async function run() {
  ...
  // Also clear old system_actions for these driver IDs
  await db.from('system_actions').delete().in('target_id', ALL_D_IDS);
  ok('Old system_actions for simulation drivers cleared');
  ...
  if (mariaContacted) {
    ...
    log('  [GAP-2] Maria (availability=not_yet) was NOT auto-contacted (deduped or not shortlisted above threshold).');
  }
  ...
  if (n5) {
    ...
    if (!n5contacted) {
      ...
      log('  [GAP-3] N5 had at least one contact — dedup did not fully block emergency need.');
    }
  }
  ...
  if (followUpSentCount === 0) {
    ...
    log(`  [GAP-5] Follow-up sent to ${followUpSentCount} driver(s). Dedup window may be too wide.`);
  }
...
run()
  .then(results => {
    const outPath = path.join(__dirname, 'sim-full-pipeline-results.json');
